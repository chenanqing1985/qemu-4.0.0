...
#define CONFIG_VIRGL     added by caq for read source code
#ifdef CONFIG_VIRGL
                                     ...
                                     struct virtio_gpu_ctrl_command *cmd)
{
    ...
    trace_virtio_gpu_cmd_ctx_create(cc.hdr.ctx_id,
                                    cc.debug_name);
//根据cmd填充成cc，然后根据cc来创建上下文
    virgl_renderer_context_create(cc.hdr.ctx_id, cc.nlen,
                                  cc.debug_name);//参照https://virgil3d.github.io/，实现在库中
}
                                ...
                                struct virtio_gpu_ctrl_command *cmd)
{
    ...
    if (virtio_gpu_stats_enabled(g->conf)) {
        g->stats.req_3d++;//3d请求计数
        g->stats.bytes_3d += cs.size;
    }

    virgl_renderer_submit_cmd(buf, cs.hdr.ctx_id, cs.size / 4);//该函数实现在库中
...
//参照 virtio_gpu_simple_process_cmd 两者功能类似，入口都是 virtio_gpu_process_cmdq
                                      ...
                                      struct virtio_gpu_ctrl_command *cmd)
{
    ...
    case VIRTIO_GPU_CMD_SUBMIT_3D:
        virgl_cmd_submit_3d(g, cmd);//给宿主机下发3D渲染请求
        break;
    case VIRTIO_GPU_CMD_TRANSFER_TO_HOST_2D:
        virgl_cmd_transfer_to_host_2d(g, cmd);
        break;
    case VIRTIO_GPU_CMD_TRANSFER_TO_HOST_3D:
        virgl_cmd_transfer_to_host_3d(g, cmd);//将cmd转换为3D请求
        break;
    case VIRTIO_GPU_CMD_TRANSFER_FROM_HOST_3D:
        virgl_cmd_transfer_from_host_3d(g, cmd);//
        break;
    case VIRTIO_GPU_CMD_RESOURCE_ATTACH_BACKING:
        virgl_resource_attach_backing(g, cmd);//前后端数据结构关联
        break;
    case VIRTIO_GPU_CMD_RESOURCE_DETACH_BACKING:
        virgl_resource_detach_backing(g, cmd);//virtio-gpu前后端数据结构解关联
        break;
...
static void virtio_gpu_fence_poll(void *opaque)
{
    ...
    virgl_renderer_poll();
    virtio_gpu_process_cmdq(g);//它还是会调用到virtio_gpu_simple_process_cmd

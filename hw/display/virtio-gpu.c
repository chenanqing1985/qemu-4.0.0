...
//根据features 决定打开 use_virgl_renderer 与否
...
//就是将virio-gpu的前端(也就是虚机)res->iov中的数据转换成宿主机可识别的res->image数据
                                           ...
                                           struct virtio_gpu_ctrl_command *cmd)
{
    ...
    VIRTIO_GPU_FILL_CMD(t2d);//从队列中取出命令字附加的信息，主要是找到资源ID
    virtio_gpu_t2d_bswap(&t2d);
    trace_virtio_gpu_cmd_res_xfer_toh_2d(t2d.resource_id);

    res = virtio_gpu_find_resource(g, t2d.resource_id);//从VirtIOGPU维护的资源链表中找到资源ID对应的图片资源
    if (!res || !res->iov) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: illegal resource specified %d\n",
    ...
    if (t2d.offset || t2d.r.x || t2d.r.y ||
        ...
        void *img_data = pixman_image_get_data(res->image);//获取主机侧图形库中图片资源的像素地址
        for (h = 0; h < t2d.r.height; h++) {
            ...
            iov_to_buf(res->iov, res->iov_cnt, src_offset,
                       (uint8_t *)img_data
                       + dst_offset, t2d.r.width * bpp);//将res->iov中的数据拷贝到res->image中
        }
    } else {
                                      ...
                                      struct virtio_gpu_ctrl_command *cmd)
{
    ...
    VIRTIO_GPU_FILL_CMD(rf);//取命令字附加信息
    virtio_gpu_bswap_32(&rf, sizeof(rf));
    trace_virtio_gpu_cmd_res_flush(rf.resource_id,
                                   rf.r.width, rf.r.height, rf.r.x, rf.r.y);

    res = virtio_gpu_find_resource(g, rf.resource_id);//查找图片资源
    if (!res) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: illegal resource specified %d\n",
    ...
    for (i = 0; i < g->conf.max_outputs; i++) {
        ...
        pixman_box16_t *extents;//将图片资源中关于像素的信息搜集起来，放到extents中
        ...
        dpy_gfx_update(g->scanout[i].con,
                       ...
                       extents->y2 - extents->y1);//送显
//将extents中的信息输出到显示器，这里的显示器驱动可以有不同实现，比如vnc，spice，sdl，gtk。virtio-gpu层根据具体驱动调用对应的实现
                                   ...
                                   struct virtio_gpu_ctrl_command *cmd)
{
    ...
    VIRTIO_GPU_FILL_CMD(ab);//从队列上取出信息，主要是获取要attach的图片资源ID。
    virtio_gpu_bswap_32(&ab, sizeof(ab));
    trace_virtio_gpu_cmd_res_back_attach(ab.resource_id);

    res = virtio_gpu_find_resource(g, ab.resource_id);//根据资源ID从VirtIOGPU设备上找到图片资源。
    if (!res) {
        qemu_log_mask(LOG_GUEST_ERROR, "%s: illegal resource specified %d\n",
...
//图片内容绑定到资源，将像素数据所在内存保存，通过dma映射的方式使用这段内存，当后端处理2D图片时就去这个地方取像素数据
                                          ...
                                          struct virtio_gpu_ctrl_command *cmd)
{
    ...
    switch (cmd->cmd_hdr.type) {//从virtio队列中取出具体的命令字，填充到cmd->cmd_hdr中
    ...
    case VIRTIO_GPU_CMD_RESOURCE_FLUSH:
        virtio_gpu_resource_flush(g, cmd);//pixman处理维护的像素数据传给终端设备
        break;
    case VIRTIO_GPU_CMD_TRANSFER_TO_HOST_2D:
        virtio_gpu_transfer_to_host_2d(g, cmd);//前端的iov结构转换为主机侧的图形库接口可以识别的结构体
        break;
    case VIRTIO_GPU_CMD_SET_SCANOUT:
        virtio_gpu_set_scanout(g, cmd);
        break;
    case VIRTIO_GPU_CMD_RESOURCE_ATTACH_BACKING:
        virtio_gpu_resource_attach_backing(g, cmd);//实际上就是映射，告诉qemu，在像素操作时去前端知会的那个地方去操作
        break;
    case VIRTIO_GPU_CMD_RESOURCE_DETACH_BACKING:
        virtio_gpu_resource_detach_backing(g, cmd);//有绑定就有解绑
        break;
    default:
        cmd->error = VIRTIO_GPU_RESP_ERR_UNSPEC;
        break;
...
{
    VirtIOGPU *g = VIRTIO_GPU(vdev);
    qemu_bh_schedule(g->ctrl_bh);//类似于linux，触发一个下半部动作,g->ctrl_bh在 virtio_gpu_device_realize 中注册
}
...
//3D的是 virtio_gpu_virgl_process_cmd处理，普通的是virtio_gpu_simple_process_cmd处理
        /* process command */
...
{
    VirtIOGPU *g = opaque;
    virtio_gpu_handle_cursor(&g->parent_obj, g->cursor_vq);//真正处理的函数
}
...
static void virtio_gpu_device_realize(DeviceState *qdev, Error **errp)
{
    ...
    g->use_virgl_renderer = false;//默认初始化为false
...
//首先明白，virtio_gpu也是一种虚拟设备，需要满足vdc的定义
static void virtio_gpu_class_init(ObjectClass *klass, void *data)
{
    ...
    vdc->unrealize = virtio_gpu_device_unrealize;//资源销毁
    vdc->get_config = virtio_gpu_get_config;//获取配置
    vdc->set_config = virtio_gpu_set_config;//设置配置
    vdc->get_features = virtio_gpu_get_features;//获取特性
    vdc->set_features = virtio_gpu_set_features;//设置特性
...
static const TypeInfo virtio_gpu_info = {
    ...
    .instance_init = virtio_gpu_instance_init,//一个类型可能有多个实例
    .class_init = virtio_gpu_class_init,//类型注册
};
